...
A production-quality, modular **Retrieval-Augmented Generation (RAG)** chatbot designed specifically to read, parse, index, and answer questions about an uploaded PDF resume. The application is built using **Python 3.11, Streamlit, LangChain, Groq APIs (Llama 3), HuggingFace/OpenAI Embeddings, and ChromaDB** as a persistent local vector database.
...
├── .env                       # Local configurations (contains Groq API key)
│
...
│   ├── 3_embeddings.py        # Embeddings generation (HuggingFace / OpenAI)
...
│   ├── 7_llm.py               # Groq LLM client initialization (Llama-3)
...
graph TD
    ...
    C --> D[3_embeddings.py: Embeddings]
...
3. **Embeddings Generation:** `3_embeddings.py` passes the text chunks to an embedding model (HuggingFace by default or OpenAI), converting them into vector embeddings.
...
7. **LLM Execution:** `7_llm.py` routes the payload to a Groq LLM model (e.g. `llama-3.3-70b-versatile`) with a temperature of `0.0` for highly deterministic, factual answers.
...
- An active Groq API account with API keys (and optionally OpenAI for embeddings).
...
```env
GROQ_API_KEY=gsk_...
# Optional: OPENAI_API_KEY=sk-proj-...
```
